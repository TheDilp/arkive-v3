...
export default function ScreenView() {
  ...
  return (
    <div className="flex w-full max-w-full gap-x-4 overflow-auto p-4">
      {sections.map((section, sectionIndex) => (
        <div key={section.id} className="flex max-w-xs flex-1 flex-col gap-y-2 ">
          ...
          <div className="flex w-full max-w-full flex-col gap-y-4">
            {(section.cards || []).map((card, cardIndex) => (
              <div key={card.id} className="w-full bg-zinc-800">
                ...
                {card?.expanded ? (
                  <div className="max-h-64 overflow-auto">{card?.content ? <StaticRender content={card.content} /> : null}</div>
                ) : null}
              </div>
            ))}
          </div>

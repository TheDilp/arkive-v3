...
import { columnsItems, defaultColumnProps, defaultSlashItems, toastError, toastWarn } from "../../../utils/utils";
...
export function CommandMenu() {
  ...
  useEffect(() => {
    if (change?.query?.full) {
      if (itemsType === "commands") {
        setItems(
          defaultSlashItems.filter((item) => item.name.toLowerCase().startsWith(change.query.full.toLowerCase())),
        );
        ...
        setItems(
          maps
            ?.filter(
              (map) =>
                !map.folder && map.title.toLowerCase().includes(change.query.full.replace("map", "").toLowerCase()),
            )
            .map((map) => ({
              name: map.title,
              type: "map",
        ...
        setItems(
          boards
            ?.filter(
              (board) =>
                !board.folder &&
                board.title.toLowerCase().includes(change.query.full.replace("board", "").toLowerCase()),
            )
            .map((board) => ({
              name: board.title,
              type: "board",
  ...
  return (
    <FloatingWrapper
      ...
      <ul className="p-0 overflow-y-auto remirror-mention-atom-popup-wrapper z-5 h-15rem" {...getMenuProps()}>

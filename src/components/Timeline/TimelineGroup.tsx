...
export default function TimelineGroup({ year, events, viewSettings }: Props) {
  ...
  return (
    <div className="flex h-fit max-h-full min-h-[24rem] w-full border-r border-zinc-700 first:border-t last:border-b even:border-y">
        ...
        {groupByHour
          ? items.map((item) => (
              <div key={item} className="flex h-full flex-1 justify-start border-b border-zinc-800">
                <div className="w-[3.5rem] border-r border-zinc-800 px-2 py-1 text-center">{item + year}</div>
